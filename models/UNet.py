...
class ConvBlock(tf.keras.layers.Layer):
    ...
    def __init__(self, filters: int, kernel_size: tuple[int, int, int],
                 use_batch_norm: bool = True, activation: tf.keras.layers.Activation | str = tf.keras.layers.ReLU,
                 name: str = "", **kwargs):
...
class UNet(tf.keras.models.Model):
    ...
    def __init__(self, **kwargs):
        ...
        self.e1_downsample = tf.keras.layers.MaxPool3D(pool_size=(2, 2, 2), name="max_pool_1")

        self.e2_l1 = ConvBlock(64, (3, 3, 3), name="e2_l1")
        self.e2_l2 = ConvBlock(128, (3, 3, 3), name="e2_l2")

        self.e2_downsample = tf.keras.layers.MaxPool3D(pool_size=(2, 2, 2), name="max_pool_2")

        self.e3_l1 = ConvBlock(128, (3, 3, 3), name="e3_l1")
        self.e3_l2 = ConvBlock(256, (3, 3, 3), name="e3_l2")

        self.e3_downsample = tf.keras.layers.MaxPool3D(pool_size=(2, 2, 2), name="max_pool_3")

        self.bottom_l1 = ConvBlock(256, (3, 3, 3), name="bottom_l1")
        self.bottom_l2 = ConvBlock(512, (3, 3, 3), name="bottom_l2")

        self.d3_upsample = tf.keras.layers.UpSampling3D(size=(2, 2, 2), name="d3_upsample")
        self.concatenate_3 = tf.keras.layers.Concatenate(axis=-1, name="concatenate_3")

        self.d3_l1 = ConvBlock(256, (3, 3, 3), name="d3_l1")
        self.d3_l2 = ConvBlock(128, (3, 3, 3), name="d3_l2")

        self.d2_upsample = tf.keras.layers.UpSampling3D(size=(2, 2, 2), name="d2_upsample")
        self.concatenate_2 = tf.keras.layers.Concatenate(axis=-1, name="concatenate_2")

        self.d2_l1 = ConvBlock(128, (3, 3, 3), name="d2_l1")
        self.d2_l2 = ConvBlock(64, (3, 3, 3), name="d2_l2")

        self.d1_upsample = tf.keras.layers.UpSampling3D(size=(2, 2, 2), name="d1_upsample")
        self.concatenate_1 = tf.keras.layers.Concatenate(axis=-1, name="concatenate_1")
